print("Zimbabwe Civ Related")

include("IconSupport.lua")
local ZimbCiv = GameInfoTypes.CIVILIZATION_ZIMBABWE_LS_MOD;
local ZimbBID = GameInfoTypes.BUILDING_ZIMBABWE;
IconHookup(GameInfo.Buildings[ZimbBID].PortraitIndex, 64, GameInfo.Buildings[ZimbBID].IconAtlas, Controls.MyImage2 )
IconHookup(2, 64, "CIV_MOD_LS_5_ATLAS", Controls.MyImage )
local CTCZ = {}
local PTCZ = {}
local uadummy = GameInfoTypes.BUILDING_DUMMY_ZIMBABWE
local ubdummy = GameInfoTypes.BUILDING_DUMMY_ZIMTRAITUB

function ZimbCheck(iPlayer)
	if Players[iPlayer]:GetCivilizationType() == ZimbCiv then
		if Players[iPlayer]:GetNumCities() > 0 then
			...
			for iCity in Players[iPlayer]:Cities() do
				...
				local numL = math.min(15, math.ceil(math.min(math.max(1, 50 / gCity), 3) * gLinked))
...
function UpdateButton()
	...
	if sCity then
		...
		Controls.MyButton2:SetText("x" .. nBonus .. "[ICON_FOOD], [ICON_TOURISM] from Births")
		local xBonus = nBonus / 20
		Controls.MyImage2:SetAlpha(xBonus)
...
if Players[Game.GetActivePlayer()]:GetCivilizationType() == ZimbCiv then
